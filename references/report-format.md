...
统一推荐路径：**HTML/CSS 生成 16:9 横版图，再用 headless browser 渲染为 PNG**。不要把 PIL 手动画图、模型原生生图或 prompt-only 当成默认实现；浏览器渲染不可用时，才返回 HTML 文件或 prompt-only 降级方案。

支持模式：

- `summary_dashboard`：默认推荐的频道横版总结图。
- `data_table`：关键数据表格图，字段来自 `【02｜关键数据卡】` 与同一证据集。
- `tokenomics_chart`：Tokenomics 图；数据完整时画分配图，数据不完整时画披露状态图，不伪造 Team / Investors / Ecosystem 占比。

图片规则：

- 图片基于 `【01｜一分钟速览】` 和 `【02｜关键数据卡】`，不使用私人决策备注。
- 三种模式都保持和 `summary_dashboard` 一致的深色 dashboard / card-grid 风格。
- 图片头部保持干净：只放标题和简短副标题；不要在右上角加装饰性 badge / pill 框，例如 `HTML/CSS 16:9`、`Public Visual`、`Data Table`、`Tokenomics`、`No Fake Ratio`。
...
- 返回前检查中文清晰度、无明显重叠/裁切、页脚完整；尤其注意 `overflow:hidden` 导致底部文字静默裁切。
