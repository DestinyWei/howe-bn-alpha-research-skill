...
- `references/`：数据源、报告格式、公共总结图规则、盘前监控工具接入说明。
...
默认不生成图片。只有用户明确要求“总结图 / 配图 / 数据表格 / 图表 / public visual”时才生成。

推荐实现：使用确定性的 HTML/CSS 生成 16:9 横版图，再用 headless browser 渲染为 PNG。图片内容应来自 `【01｜一分钟速览】` 和 `【02｜关键数据卡】`，不包含私人决策备注。
